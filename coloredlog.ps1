...
Write-Host "	+" -f Magenta -NoNewline; Write-Host "`tNerdAtWork's ColoredLog" -f Gray -NoNewline; Write-Host "`t`t+" -ForegroundColor Magenta
...
switch($choice){
   1 {$containername = Read-Host -Prompt "`tEnter container name (ex:storagenode, storagenode1)"            
   ...
   2 {$pathtolog = Read-Host -Prompt "`tEnter path to log file with filename"
